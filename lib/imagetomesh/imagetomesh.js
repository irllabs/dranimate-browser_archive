...
*****************************/
    ...
    this.eraseUnselectedPixels = function () {
...
*****************************/
    ...
    this.doSLICOnImage = function () {
        ...
        slic = new SLIC(originalImageData, { method: "slic", regionSize: 20 });
    ...
    this.generateTriangles = function() {
        ...
        console.error("NOTE: Big optimization here boyyyy !!!! ~~~")
...
*****************************/
    ...
    canvas.addEventListener('mousemove', function(evt) {
        ...
        mouse.x = mouseAbs.x - pan.x;
        mouse.y = mouseAbs.y - pan.y;
        ...
        if(panEnabled) {
            if(mouse.leftClickDown) {
                pan.x += mouseAbs.x - panFromPosition.x;
                pan.y += mouseAbs.y - panFromPosition.y;
    ...
    canvas.addEventListener('mousedown', function(evt) {
        ...
        if(evt.which == 3) {
            if(!panEnabled) that.removeSelectionToNoBackgroundImage();
            mouse.rightClickDown = true;
        } else {
            if(!panEnabled) that.addSelectionToNoBackgroundImage();
